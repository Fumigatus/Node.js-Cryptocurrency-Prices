    ...
    async getPrice(coinO, curO, intervalO) {
        try {
            ...
            res.data.forEach(coin => {
                ...
                output += `Coin name: ${coin.name.cyan} ${('(' + coin.symbol + ')').cyan}, Price (${curO.toUpperCase()}): ${prcChange > 0 ? price.green : price.red}, ${interval} price change: ${prcChange > 0 ? prcChange.green : prcChange.red} / ${pctChange > 0 ? pctChange.green : pctChange.red}% , Rank: ${coin.rank.blue}\n`.bold
            });
            return output
        } catch (err) {
            console.log(`${err}`)
